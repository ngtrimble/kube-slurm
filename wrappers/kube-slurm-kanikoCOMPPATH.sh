...
# Kaniko requirement
  ...
  - name: kaniko-secret
    secret:
      secretName: bwhcomppathkanikocred
      items:
  ...
  containers:
  - name: ${KUBE_JOB_NAME}
    image: erisxdl.partners.org/library/kaniko-project/executor:debug
    #image: erisxdl.partners.org/library/kaniko-project/executor:latest
    args:
#      - "--dockerfile=Dockerfile"
      - "--dockerfile=${DOCKERFILE}"
      - "--context=dir:///workspace"
#      - "--destination=erisxdl.partners.org/bwh-comppath-kaniko/kaniko-test:v1"
      - "--destination=erisxdl.partners.org/${KANIKO_DEST_GRP_ACCT}/${KANIKO_DEST_IMAGE}"
#    hostNetwork: true - fails
    env:
    - name: HOME
      value: "${USER_HOME}"
    - name: DOCKER_CONFIG
      value: /kaniko-secret/.docker
    volumeMounts:
    - mountPath: /etc/apt/apt.conf.d/01proxy
      name: etc
      subPath: apt/apt.conf.d/01proxy
    - mountPath: /etc/hosts
      name: etc
      subPath: hosts
    - name: context
      mountPath: /workspace
    - name: kaniko-secret
      mountPath: /kaniko-secret/.docker
    - name: data
