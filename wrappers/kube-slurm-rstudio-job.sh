#!/bin/bash
## Manage rstudio Job Pods from Slurm
...
KUBE_CLUSTER_DNS=${KUBE_CLUSTER_DNS:=rstudio.erisxdl.partners.org}
KUBE_INGRESS_PREFIX="/rstudio"
KUBE_TARGET_PORT=${KUBE_TARGET_PORT:=8787}
USER_HOME=${HOME}
...
## Create Service
log "Creating RStudio Session ClusterIP Service"
...
RSTUDIO_TOKEN=$(openssl rand -hex 24)
log "Generated RStudio Token: '${RSTUDIO_TOKEN}'"
...
spec:
  tls:
  - hosts:
# Using SAN, Subject Alternative Name on the cryo ssl.
    - rstudio.erisxdl.partners.org
    secretName: cryosparc-2024-erisxdl-partners-org
  rules:
...
## Generate Session URL (Ingress)
RSTUDIO_URL="https://${KUBE_CLUSTER_DNS}?token=${RSTUDIO_TOKEN}"
echo "########################################################"
echo "Your RStudio session URL will be: ${RSTUDIO_URL}"
echo "########################################################"
...
spec:
  ...
  - name: ${KUBE_JOB_NAME}
    ...
    args:
      - "-c"
      # Note: $USER is key for authentication
      - "export USER=$(whoami); rserver --server-user=${USER} --server-working-dir=/home/${USER}  \
          --server-pid-file=/home/${USER}/var/run/rstudio-server.pid  \
          --server-data-dir=/home/${USER}/run/rstudio-server; sleep infinity"
    volumeMounts:
    - name: data
      mountPath: /work/data
    - name: home
